...
def main():
    ...
    if args.local_rank == 0:
        _logger.info('Model %s created, param count: %d  trainable params: %d' %
                     (args.model, sum([m.numel() for m in model.parameters()]),sum([m.numel() for m in model.parameters() if m.requires_grad])))
